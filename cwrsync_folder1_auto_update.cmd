...
echo ===START cars auto update >> cwrsync_log.txt
setlocal
...
rsync -chavvzPu --stats --exclude-from={'cwrsync-exclude.txt'} username1@1.1.1.1:/rootpath1/path2/path3/folder1/ folder1/  

@echo 'fin'
echo ===FINISHED folder1 auto update >> cwrsync_log.txt
setlocal
for /f "tokens=*" %%f in ('tzutil /g') do (
  echo %DATE%     %TIME%     ^(%%f^) >> cwrsync_log.txt
  )
